...
def main():
    ...
    img_details = [row[0].astype(str) for row in matrices]  # Seems like the first row is a code
    img_details2 = [row[1].astype(str) for row in matrices]  # Seems like the first row is a code
    ...
    for i in xrange(num_images):
        ...
        image_details.append(_img_dict)

    count = 0
    for i, k in zip(img_details, img_details2):
        print '--------------------------------------------------------------------------------'
        ...
        print
        count += 1
        map(update_img_in_mongo, image_details)
